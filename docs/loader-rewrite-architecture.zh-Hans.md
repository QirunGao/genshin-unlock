...
本节基于当前仓库代码重新对照本文档。与早期版本相比，文末原先那批“不一致点”里，已有相当一部分已经落地，因此这里改为区分：

- 哪些关键链路已经与本文档一致
- 哪些地方仍然只是“部分达成”或仍需继续一致化

结论先行：

- 当前实现已经不只是“新架构骨架”
- 启动链路、握手阻断、配置快照传递、最小化 `DllMain`、严格版本匹配、最小监控循环这几项已经落地
- 剩余差异主要集中在可观测性深度、状态机完整性、Hook 生命周期收口和信任校验闭环
...
1. `launcher.exe` 以挂起方式启动游戏
2. 通过 `CreateRemoteThread + LoadLibraryW` 注入 `bootstrap.dll`
3. 显式远程调用 `BootstrapEntryPoint`
4. `bootstrap.dll` 建立 IPC、接收 `Hello` 与 `ConfigSnapshot`
5. `bootstrap.dll` 加载 `runtime.dll` 并调用 `RuntimeInitialize(params)`
6. `launcher.exe` 仅在收到 `BootstrapReady` 与 `RuntimeInitResult=Ok` 后恢复主线程

这意味着，文档里旧版“只完成 DLL 映射、没有真正初始化 bootstrap”的说明已经不再适用。

#### 21.1.2 启动流程已经改成 fail-closed

当前 `launcher` 在以下任一阶段失败时都会终止目标进程并停止启动：

- `bootstrap.dll` 注入失败
- `BootstrapEntryPoint` 远程调用失败
- IPC 建连失败
- `BootstrapReady` 接收失败或状态非 `Ok`
- 配置快照发送失败
- `RuntimeInitResult` 接收失败或状态非 `Ok`

也就是说，文档里旧版“握手失败仍然放行并 `ResumeThread()`”的说明已经过时。

#### 21.1.3 IPC 建连时序已经补齐

当前 `bootstrap` 初始化中已经明确执行了：

1. `CreateNamedPipeA`
2. `WaitForConnection()`
3. `ReceiveHello(...)`
4. `SendBootstrapReady(...)`
5. `ReceiveConfigSnapshot(...)`

因此，文档里旧版“只创建了管道、没有真正等待连接”的说明也已经过时。

#### 21.1.4 配置快照已经通过引导链传入 `runtime`

当前实现里：

- `launcher` 负责读取 `launcher_config.json` 与 `runtime_config.json`
- `launcher` 构造 `ConfigSnapshotMessage`
- `bootstrap` 接收配置后构造 `RuntimeInitParams`
- `runtime` 从 `params->config` 应用初始配置

因此，文档里旧版“`bootstrap` 仍然调用 `RuntimeInitialize(nullptr)`、`runtime` 依赖文件态配置启动”的说明已经过时。

#### 21.1.5 IPC 协议已收敛为固定大小 POD 消息

当前 `shared/Protocol.hpp` 中的消息结构已经全部改成固定大小字段：

- 不再依赖 `std::string`
- 通过 `MessageHeader + payloadSize + POD payload` 直接读写
- `HandshakeClient`、`IpcServer` 与 `IpcWriter` 都按同一套大小约定处理消息

这意味着，文档里旧版“协议定义仍包含复杂字符串结构、实现与定义脱节”的说明已经不再准确。

#### 21.1.6 运行时配置文件读写已经从被注入进程移出

当前 `runtime.dll` 已经不再读取或回写 `runtime_config.json`。热键切换只更新运行时内存态配置，不再直接做 JSON 持久化。

这一点已经与本文档“配置解析尽量外移”的方向一致。当前游戏版本读取也已经完全留在 `launcher.exe` 侧，`runtime.dll` 只接收控制面传入的版本字符串。

#### 21.1.7 版本匹配已经改为严格白名单

当前 `VersionTable::IsSupported()` 和 `GetProfile()` 都要求完整版本号精确匹配，不再使用 `major/minor` 模糊匹配。

因此，文档里旧版“同主次版本会被宽松视为已支持”的说明已经过时。

#### 21.1.8 `launcher` 已具备最小监控面

当前在恢复游戏主线程后，`launcher` 已经可以在监控循环中接收和记录：

- `StatusHeartbeat`
- `HookStateChanged`
- `ErrorEvent`

因此，文档里旧版“恢复主线程后只等待 3 秒并退出、没有监控面”的说明也已经过时。

### 21.2 当前实现已与本文档核心设计对齐

截至当前版本，本文档前文要求的关键能力都已经在代码中形成闭环：

- `launcher -> bootstrap -> runtime` 的显式初始化链路
- fail-closed 启动与握手阻断
- 握手阶段的显式超时控制
- 配置快照下发、配置生效确认与初始启停语义一致
- `RuntimeLoadRequest` 驱动的 runtime 路径与哈希二次校验
- 控制面传入游戏版本
- 固定大小 POD IPC 协议
- `ControlPlaneReady` 就绪确认后再恢复游戏主线程
- `StateChanged / StatusHeartbeat / HookStateChanged / LogEvent / ErrorEvent`
  观测链路
- `ShutdownRequest` 优雅关闭链路
- `HookManager` 的统一注册、安装、启停、卸载
- 运行态热键切换与 `HookManager` 状态同步
- 能力级降级：单项 Hook 失败不会回滚全部能力
- FOV 状态收口到服务对象内部
- 运行时拆分为显式初始化、控制面启动、输入更新路径、心跳路径与控制路径
- 初始化阶段状态转移延迟回放到控制面，`Fatal` 进入点已补齐
- 基于 `module_hashes.json` 的哈希白名单 fail-closed 校验
- `launcher.exe` 负责日志落盘

因此，当前仓库实现已经可以视为与本文档描述的核心架构目标一致。

### 21.3 剩余事项的性质

当前仍可继续增强的内容，已经主要不再属于“主链路不一致”，而属于后续强化项，例如：

- 发布阶段增加 Authenticode 签名校验
- 为 IPC 与注入流程增加更高层级的集成测试
- 根据需要扩展更多协议消息、诊断字段或在线配置更新能力
- 继续优化 Hook 底层抽象

这些属于增强项，不影响当前实现与本文档核心设计的一致性。

## 22. 当前结论

当前代码已经完成本文档所描述的重写架构主闭环，并补齐了此前 review 中暴露出的几个关键缺口：控制面启动就绪确认、配置启停语义一致性、运行态 Hook 状态同步、状态机事件补发、会话与协议版本校验，以及错误事件中的阶段/错误码/系统错误闭环。本文档现在既可以作为架构说明，也仍可作为后续增强项的设计基线。
